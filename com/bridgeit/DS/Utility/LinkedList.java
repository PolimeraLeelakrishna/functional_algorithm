...
public class LinkedList<T> // T stands for generic
{
	...
	Node head; // declearing head nodes
...

	{
		...
		{
			if (n.data.equals(element)==true) // comparing data with user search value
			{
				return true;
			}
			else
...
public boolean searchele(T element) 
{ 
    @SuppressWarnings("unchecked")
	Node<T> current = head;    //Initialize current 
    while (current != null) 
